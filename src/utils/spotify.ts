...
const SPOTIFY_CLIENT_SECRET = 'fe9407e3dbe644efa0c8dc9d9bbd7496'
...
export function getArtistImage(artist: SpotifyArtist | null, size: 'large' | 'medium' | 'small' = 'large'): string | null {
  ...
  // small
  return artist.images[2]?.url || artist.images[1]?.url || artist.images[0]?.url || null
}

export async function searchSpotifyTrack(trackName: string, artistName: string): Promise<SpotifyTrack | null> {
